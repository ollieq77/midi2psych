...
:: ---- Compile manifest resource (fixes "entry point not found" from comctl32 v5 vs v6) ----
echo  [....] Compiling app manifest
:: windres forwards -I paths to its internal preprocessor without escaping spaces,
:: so a path like "...\Ollie Quayle\..." breaks it. Side-step this by cd-ing into
:: assets\ first, so app.rc's relative "app.manifest" resolves with no -I needed.
pushd "%ASSETS%"
windres -i "app.rc" --input-format=rc -O coff -o "%BUILD%\app_res.o"
set WINDRES_ERR=%errorlevel%
popd
if %WINDRES_ERR% neq 0 (
    echo  [FAIL] windres could not compile assets\app.rc
    pause & exit /b 1
)
echo.
...
g++ -std=%STD% %OPT% %WARN% %EXTRA_FLAGS% ^
    ...
    "%BUILD%\app_res.o" ^
    -lcomctl32 -lcomdlg32 -lgdi32 -lshell32 -ldwmapi -luxtheme -lole32
