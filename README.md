...
## What's new in the V2.0 GUI overhaul

The GUI got a full visual and functional rework — no more Windows-98 grey boxes.

- **Dark, custom-drawn interface** — flat rounded cards, owner-drawn buttons and checkboxes, a themed progress bar with a live percentage, and a native dark title bar / rounded corners on Windows 11.
- **Drag & drop** — drop MIDI files straight onto the window; the first drop fills P1, the second fills P2.
- **Instant MIDI analysis** — as soon as you pick a file (or hit *Analyze*), the app reads it in the background and shows track count, note count, detected tempo and duration.
- **Swap P1 / P2** — one click to flip both the files and the character names. Handy when you loaded them backwards.
- **Presets** — Save Preset / Load Preset writes all your conversion settings to a small `.m2p` file so you can reuse a setup across songs.
- **Open Output Folder** — jump straight to the converted chart after a run.
- **Inline validation** — bad numeric input highlights the field in red instead of just popping a dialog.
- **Non-blocking toast notifications** — success/error messages slide in without stealing focus like the old modal popups did.
...
midi2psych/
  ...
  include/              Header files
    theme.h             Dark UI palette + GDI drawing helpers (rounded rects, buttons)
  scripts/
