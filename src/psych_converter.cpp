...
// ─── buildPatternNotes (Pattern Mode) ─────────────────────────────────────────

std::vector<MIDINote> PsychConverter::buildPatternNotes(
    ...
    const std::function<void(double)>& progressCallback) const {

    auto report = [&](double p) { if (progressCallback) progressCallback(p); };

    std::vector<MIDINote> result;
    report(0.0);

    // The master file is a trigger track, not a chart — parse it with
    // sustainNotes=false regardless of the user's sustain setting (note-off
    // timing on the master doesn't mean anything; only note-on tick + pitch do).
    MIDIParser masterParser;
    if (!masterParser.parse(masterFile, false, minVelocity)) {
        guiLogger.logColored("\n[X] Pattern Mode: couldn't read master MIDI: " + masterFile + "\n", RED);
        return result;
    }
    report(0.05);

    outPpq   = masterParser.ppq;
    outBpm   = masterParser.bpm;
    outTempo = masterParser.tempoChanges;

    // Merge every track's trigger notes into one tick-ordered list (a track's
    // own events are already tick-ascending, but multiple tracks need merging).
    std::vector<MIDINote> triggers;
    for (auto& track : masterParser.tracks)
        for (auto& n : track) triggers.push_back(n);
    std::sort(triggers.begin(), triggers.end(),
        [](const MIDINote& a, const MIDINote& b) { return a.tick < b.tick; });

    if (triggers.empty()) {
        guiLogger.logColored("\n[!] Pattern Mode: master MIDI has no trigger notes.\n", YELLOW);
        report(1.0);
        return result;
    }

    // Work out which distinct pattern indices are actually referenced, up
    // front, purely so progress has an accurate denominator (parsing each
    // pattern file is the slow part; the splice loop itself is cheap).
    std::vector<int> neededIdx;
    for (auto& trig : triggers) {
        if (trig.note < 60) continue;
        int idx = static_cast<int>(trig.note) - 60;
        if (idx < 0 || static_cast<size_t>(idx) >= patternFiles.size() || patternFiles[idx].empty())
            continue;
        if (std::find(neededIdx.begin(), neededIdx.end(), idx) == neededIdx.end())
            neededIdx.push_back(idx);
    }
    size_t totalPatternsToParse = neededIdx.size();
    size_t patternsParsed = 0;

    // Parse each referenced pattern file at most once, even if several
    // triggers (or both players) reuse the same pattern index.
    std::unordered_map<int, MIDIParser> patternCache;

    // Parsing occupies 0.05 -> 0.85 of the bar; splicing occupies 0.85 -> 1.0.
    for (size_t i = 0; i < triggers.size(); ++i) {
        const auto& trig = triggers[i];
        if (trig.note < 60) continue;  // below C3 — not a pattern trigger

        int patIdx = static_cast<int>(trig.note) - 60;  // C3(60)=0 -> "pattern 1"
        if (patIdx < 0 || static_cast<size_t>(patIdx) >= patternFiles.size()
            || patternFiles[patIdx].empty()) {
            guiLogger.logColored("\n[!] Pattern Mode: no pattern assigned for trigger note "
                + std::to_string(static_cast<int>(trig.note)) + " (pattern " + std::to_string(patIdx + 1)
                + ") — skipping.\n", YELLOW);
            continue;
        }

        auto cacheIt = patternCache.find(patIdx);
        if (cacheIt == patternCache.end()) {
            MIDIParser pp;
            if (!pp.parse(patternFiles[patIdx], sustainNotes, minVelocity)) {
                guiLogger.logColored("\n[X] Pattern Mode: couldn't read pattern file: "
                    + patternFiles[patIdx] + "\n", RED);
                continue;
            }
            cacheIt = patternCache.emplace(patIdx, std::move(pp)).first;
            ++patternsParsed;
            if (totalPatternsToParse > 0)
                report(0.05 + (static_cast<double>(patternsParsed) / totalPatternsToParse) * 0.80);
        }
        MIDIParser& pat = cacheIt->second;
        if (pat.ppq == 0) continue;

        // Patterns can use any PPQ — rescale their ticks into the master's
        // tick-space. Only relative timing matters; a pattern's own tempo
        // track (if any) is ignored, since patterns are note shapes, not songs.
        double tickScale = static_cast<double>(outPpq) / static_cast<double>(pat.ppq);

        // Cut the pattern off at the NEXT trigger, whichever comes first:
        // another master note, or the pattern simply running out of notes.
        uint32_t cutoffTick = (i + 1 < triggers.size()) ? triggers[i + 1].tick
                                                         : UINT32_MAX;

        for (auto& track : pat.tracks) {
            for (auto& n : track) {
                uint32_t shiftedTick = trig.tick
                    + static_cast<uint32_t>(std::llround(n.tick * tickScale));
                if (shiftedTick >= cutoffTick) continue;
                uint32_t shiftedDur = static_cast<uint32_t>(std::llround(n.duration * tickScale));
                result.emplace_back(shiftedTick, n.note, n.velocity, shiftedDur);
            }
        }

        if ((i & 0x3F) == 0)  // throttle: only every 64 triggers, splicing itself is cheap
            report(0.85 + (static_cast<double>(i) / triggers.size()) * 0.15);
    }
...
bool PsychConverter::convert(const std::string& p1File,
    ...
    // In Pattern Mode, p1File/p2File are treated as MASTER (trigger) MIDIs and
    // the real notes come from splicing in the matching pattern files instead
    // of the master's own tracks — buildPatternNotes() does that and we drop
    // the result straight into p1Parser/p2Parser so every line below this
    // block (lane assignment, sorting, section building, JSON) is unchanged
    // and doesn't know or care whether Pattern Mode was involved.
    auto parseP1 = [&]() -> bool {
        if (m_config.patternMode) {
            uint16_t ppq; double bpmVal; std::vector<TempoChange> tempo;
            auto notes = buildPatternNotes(p1File, m_config.patternFilesP1,
                m_config.sustainNotes, m_config.minVelocity, ppq, bpmVal, tempo,
                [&](double p) {
                    p1Pct.store(static_cast<int>(p * 100), std::memory_order_relaxed);
                    parseBar.update((p1Pct.load() + p2Pct.load()) * 0.005,
                        "P1 patterns: " + std::to_string(p1Pct.load()) + "% | P2: " + std::to_string(p2Pct.load()) + "%");
                });
            if (notes.empty()) return false;
            p1Parser.tracks = { std::move(notes) };
            p1Parser.ppq = ppq; p1Parser.bpm = bpmVal; p1Parser.tempoChanges = std::move(tempo);
            return true;
        }
    ...
    auto parseP2 = [&]() -> bool {
        if (m_config.patternMode) {
            uint16_t ppq; double bpmVal; std::vector<TempoChange> tempo;
            auto notes = buildPatternNotes(p2File, m_config.patternFilesP2,
                m_config.sustainNotes, m_config.minVelocity, ppq, bpmVal, tempo,
                [&](double p) {
                    p2Pct.store(static_cast<int>(p * 100), std::memory_order_relaxed);
                    parseBar.update((p1Pct.load() + p2Pct.load()) * 0.005,
                        "P1: " + std::to_string(p1Pct.load()) + "% | P2 patterns: " + std::to_string(p2Pct.load()) + "%");
                });
            if (notes.empty()) return false;
            p2Parser.tracks = { std::move(notes) };
            p2Parser.ppq = ppq; p2Parser.bpm = bpmVal; p2Parser.tempoChanges = std::move(tempo);
            return true;
        }
        return p2Parser.parse(p2File, m_config.sustainNotes, m_config.minVelocity);
    };

    auto p1Future = std::async(std::launch::async, parseP1);
    auto p2Future = std::async(std::launch::async, parseP2);
    ...
    if (!p1Ok) {
        ...
        if (m_config.patternMode) guiLogger.logColored("    (Pattern Mode: check the master MIDI and P1 pattern list.)\n", RED);
        return false;
    }
    if (!p2Ok) {
        guiLogger.logColored("\n[X] Failed to parse P2 MIDI file!\n", RED);
        if (m_config.patternMode) guiLogger.logColored("    (Pattern Mode: check the master MIDI and P2 pattern list.)\n", RED);
        return false;
    }
